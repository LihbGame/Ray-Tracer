...
inline Hitable* World::InitMoveSphereSence()
{
	...
	for (int a = -10; a < 10; a++) {
		for (int b = -10; b < 10; b++) {
			...
			if ((center - vec3(4, 0.2, 0)).length() > 0.9) {
				if (choose_mat < 0.8) {  // diffuse
					list[i++] = new MoveSphere(center, center + vec3(0, 0.5 * random_double(), 0), 0.0, 1.0, 0.2, new Lambertian(vec3(random_double() * random_double(), random_double() * random_double(), random_double() * random_double())));
				}
				else if (choose_mat < 0.95) { // metal
...
inline void World::DrawMotionBlurSence(HDC* dc)
{
	...
	Hitable* world = InitMoveSphereSence();
	...
	MotionBlurCamera cam(lookfrom, lookat, vec3(0, 1, 0), 20, float(nx) / float(ny), aperture, dist_to_focus,0.0f,1.0f);
